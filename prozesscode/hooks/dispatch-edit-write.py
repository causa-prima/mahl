...
# Die Checks sind Paketmodule (S129). Der Präfix steht als Konstante, damit Tests denselben
# Namensraum ansprechen können, ohne ihn ein zweites Mal zu tippen – eine getippte Kopie
# ginge beim nächsten Umzug still auseinander.
PAKET = "prozesscode.hooks."
...
def collect_reasons(data: dict) -> list[str]:
    ...
    for name in CHECKS:
        try:
            reason = import_module(PAKET + name).check(data)
