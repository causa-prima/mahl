...
def _pruefe_anker(tid: str, faellig: str, root: Path | None = None) -> None:
    ...
    if fehler:
        raise ValueError(
            ...
            + f". Grammatik: prozesscode/td_anchors.py, Vorlage im Header von {TD_FILE}.")
...
def memory_entfernen(memory_text: str, tid: str) -> str:
    ...
    Der Fettdruck ist **optional**: Ausgeschriebene Punkte tragen ihn, der Platzhalter eines
    TD-Punkts (`- TD-S129-1 · Done: …`) nicht. Bis S129 verlangte das Muster ihn – die
    Funktion gab für jeden Platzhalter still den unveränderten Text zurück, während
    `td.py remove` Erfolg meldete (`memory_hat` prüft nur auf die ID). Zurück blieb ein
    verwaister Punkt, der bei jedem Session-Start vorgelegt wurde.
    """
    muster = re.compile(
        rf"^- (?:\*\*)?.*?{re.escape(tid)}.*?$(?:\n(?!\s*-\s|##)[^\n]*)*\n?", re.M)
