...
Verwendung:
  python3 -m prozesscode.stryker-frontend                              # alle Dateien
  python3 -m prozesscode.stryker-frontend --mutate src/pages/Foo.tsx   # eine Datei
  python3 -m prozesscode.stryker-frontend --mutate src/a.ts,src/b.tsx  # mehrere Dateien
  python3 -m prozesscode.stryker-frontend --verbose                    # alle nicht-getöteten Mutanten
...
_TMP_FILE     = _REPO_ROOT / ".claude" / "tmp" / "stryker_frontend_out.txt"
...
def main() -> None:
    ...
    # stryker-summary ist das maßgebliche Gate (Score < 100 % → exit 1) und deckt sich mit
    # Strykers eigenem break-Threshold. Bei Abweichung gewinnt das Fail.
    # `-m` statt Dateipfad: das Modul nutzt paketinterne Importe und läuft nur mit Paketkontext.
    summary_args = [sys.executable, "-m", "prozesscode.stryker-summary", str(report_json)]
    if args.verbose:
        summary_args.append("--verbose")
    summary_result = subprocess.run(summary_args, cwd=str(_REPO_ROOT))
