...
<!--
wann-lesen: Bevor du TypeScript/React-Produktionscode oder Tests schreibst (nach docs/guidelines/coding-guideline-general.md)
kritische-regeln:
...
- **`user-event` für Eingaben (Pflicht):** `await user.type(field, 'Tomate')` statt `fireEvent.change(...)` – simuliert echte Event-Sequenzen (zeichenweise) und deckt Bugs in Eingabe-Handlern auf, die `fireEvent` durch Direkt-Setzen des Werts durchwinkt.
- **Klicks: `fireEvent.click` ist der Default.** Es erzeugt schnelle, deterministische Assertion-Kills im Mutation-Test. `user.click` durchläuft die volle Pointer-Sequenz und hängt bei Mutanten, die ein Modal offen lassen, bis in Strykers Timeout (gemessen: ~2× Laufzeit, Kills via Timeout statt Assertion) – ohne dort Mehrwert, wenn der Test das Schließen ohnehin per Assertion prüft.
  - **Wann `user.click` gezielt einsetzen:** nur wenn der *Prüfgegenstand* ist, dass eine Interaktion **nicht** möglich sein darf, weil ein Vorfahr `pointer-events: none` trägt (z.B. blockierter Modal-Hintergrund). Verifiziert (jsdom): das ist der **einzige** Fall, in dem `user.click` einen Regress fängt, den `fireEvent.click` maskiert.
  - **Kein Differentiator:** `disabled` (beide feuern onClick nicht → mit `toBeDisabled()` prüfen, nicht per Klick) und **geometrische Überdeckung** (Toast/Spinner/Overlay liegt sichtbar über dem Button – jsdom rechnet kein Layout, fängt **keiner**). Solche Verdeckungen gehören in den **Playwright-E2E** (echter Browser mit Layout/Hit-Testing).
- **Zustandsübergänge explizit per Assertion absichern (Pflicht-Komplement zum `fireEvent.click`-Default):** Hängt eine Folge-Interaktion davon ab, dass ein vorheriger UI-Übergang abgeschlossen ist (Dialog/Overlay geschlossen, Element entfernt oder aktiviert), prüfe diesen Übergang **selbst per Assertion** – `fireEvent.click` verlässt sich nicht darauf und würde einen Regress, der den Übergang unterlässt, sonst maskieren. Beispiel: nach dem Schließen eines Dialogs **vor** der nächsten Aktion `await waitFor(() => { expect(screen.queryByRole('dialog')).not.toBeInTheDocument() })` (bzw. `queryByLabelText('…')` für ein konkretes Feld). Das ersetzt die Actionability-Prüfung, die `user.click` automatisch mitbrächte, durch eine explizite, schnelle und deterministische Assertion.
- **Hintergrund-Elemente bei offenem Dialog: versteckte explizit einschließen.** Ein MUI-`Dialog` im `open`-Zustand setzt `aria-hidden` auf den restlichen DOM-Baum; rollenbasierte Queries (`getByRole`/`queryByRole`) ignorieren versteckte Elemente per Default und finden Hintergrund-Inhalt dann **scheinbar grundlos** nicht. Müssen bei offenem Dialog/Overlay Hintergrund-Elemente über ihre Rolle abgefragt werden, `{ hidden: true }` setzen, z.B. `screen.getByRole('listitem', { hidden: true })`. (Pendant für E2E: `docs/process/e2e-testing.md`.)
- **Negativ-/„passiert-nicht"-Assertions faithful absichern (Vakuitäts-Falle).** Ein Test, der prüft, dass etwas **nicht** passiert (Dialog schließt *nicht*, Aktion bleibt gesperrt), kann **grün sein, obwohl der Guard fehlt/kaputt ist** – ein vakuöser Pass, der falsche Sicherheit gibt (Negativ-Assertionen passen bereits, wenn gar nichts verdrahtet ist). Zwei Auslöser + Gegenmittel: (1) **Kein echtes RED bei retroaktivem/emergentem Verhalten** – existiert der Guard beim Testschreiben schon (nachträgliches Spezifizieren vorhandenen Verhaltens), gibt es keine RED-Phase, die die Vakuität aufdeckt → den Guard **temporär entfernen** und bestätigen, dass der Test **rot** wird (Faithfulness-Check), dann wiederherstellen. (2) **RED-first nur mit Prüfung des richtigen Grundes** – ist ein neuer Test beim ersten Lauf grün, obwohl er rot sein müsste, ist er vakuös (nicht „schon implementiert"). MUI-Dialog-Gotchas, die konkret Vakuität erzeugen: **Escape** aus einem Element *im* Modal feuern (fällt der Fokus vom deaktivierten Button auf `<body>`, erreicht Escape den MUI-Handler nie → Test grün ohne Guard); **Backdrop-Klick** braucht `fireEvent.mouseDown(container)` *vor* `fireEvent.click` (MUI erkennt Backdrop zweistufig – `click` allein lässt die interne Ref leer); eine **„noch offen"-Assertion** braucht ein **Settle-Fenster > MUI-Exit-Transition (~225 ms)** davor, sonst sieht sie den Dialog während einer *echten* Schließ-Transition fälschlich noch im DOM.
