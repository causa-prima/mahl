...
## OBS-S125-1 – Nackte Tracker-IDs im Gespräch
- Quelle: User
- Status: UMGESETZT (S126)
- Impact: MITTEL    Häufigkeit: dauerhaft
- Kategorie: PROZESS    Kontext: Skill-Nutzung
...
- Zusammen-erledigen: keiner
- Entscheidung/Maßnahme: Gewaehlt: Prinzip + Ausgabe-Schema + Titel-Korrekturschleife, kein neues Feld. (1) principles.md, Kommunikation: "Eine Tracker-ID nie nackt nennen" – Erstnennung ID + Kurztitel + zwei, drei erklaerende Saetze, die eine qualifizierte Nachfrage erlauben; jede weitere Nennung ID + DERSELBE Kurztitel, auch wenn der User selbst nur die ID schreibt. Gilt fuer alle Tracker (OBS/LL/TD/OQ/ADR), nicht nur OBS. (2) Der Kurztitel IST der Titel des Eintrags. Traegt er den Punkt nicht, schlaegt der Agent Alternativen vor, der User entscheidet, und die Wahl wird zurueckgeschrieben – ein Name an einem Ort, der mit jeder Nennung besser wird. Dafuer gebaut: --titel in obs.py set, td.py set und oq.py set (obs_entry.set_fields, tracker_entry.set_fields), 4 Tests. (3) Knapper Verweis auf das Prinzip an den drei Stellen, an denen Eintraege vorgelegt werden: draining-observations, kaizen (CM/LL/OBS), implementing-scenario (ADR-Check, TD-Sichtung). VERWORFEN: (a) obs-drain.py schneidet das Beobachtungsfeld automatisch an – das Feld ist nicht als Pitch geschrieben, sein erster Satz ist der Einstieg der Erfassung; Beleg ist OBS-S113-1, dessen erster Satz vier IDs und eine Datei nennt und als Pitch wertlos waere. (b) Ein eigenes Kurzname-Feld am Eintrag – so lautete die Vorpraegung, der User hat sie im Drain selbst verworfen: zweiter Ort fuer denselben Namen, veraltet still bei Nachtraegen, und die Folge waere falsche statt fehlender Information. (c) Den Titel mechanisch in die session-agenda-Ausgabe von modul_td_due aufnehmen – vom Orchestrator als Analogie zu modul_open_questions befuerwortet und nach dem Einwand des Users zurueckgezogen: Das Prinzip macht den Abruf ohnehin verpflichtend, und ein vorgegebener Titel zieht zur unbesehenen Uebernahme. Die Analogie trug nicht, weil modul_open_questions in den AGENTEN-Kontext schreibt und damit eine Voraussetzung des Pitches loest, nicht den Pitch. GRENZE, bewusst getragen: "immer derselbe Kurztitel" gilt innerhalb eines Gespraechs. Sessionuebergreifend erfinden zwei Agenten zwei aehnliche Namen, solange nichts gespeichert ist; dort traegt die Erstnennung mit Pitch. Reicht das spaeter nicht, ist der Weg die Titelkorrektur, nicht ein zweites Feld. NICHT ERWEITERT: lessons.py hat gar kein set-Kommando – das gehoert zu OBS-S125-2.
