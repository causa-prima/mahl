...
| Häufige Befehls-Denies (67 `COMPOUND_UNKNOWN`, 36 seit S70) → Zeit/Token-Verlust; Ursachen ungeklärt (`cd &&` nur ~13/67) (S078) | MITTEL | TOOLING | Bash/Permission | Deny-Log `.claude/tmp/denied-commands.log` nach Ursachen kategorisieren, dann entscheiden: Hook lockern / Allow-Patterns / Agenten-Guidance (absolute Pfade) / Deny-Hint-UX (`--list` unternutzt). Datenlage reicht – kein „mehr Daten"-Trigger. | OFFEN | 078 |
