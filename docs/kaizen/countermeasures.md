...
**Impact:** MITTEL | **Kategorie:** PROZESS | **Kontext:** Skill-Nutzung | **Status:** OFFEN | **Seit:** S107
...
**Maßnahme:** Syntaktischer PreToolUse-Poka-Yoke `.claude/hooks/check-obs-capture.py` (via TDD, analog CM-S102-2 `check-ref-direction.py`): bei Edit/Write an `docs/kaizen/observations.md` muss das `- Entscheidung/Maßnahme:`-Feld eines **neu** hinzugefügten OBS mit dem Kanon-Token `offen` beginnen; ein bei Erfassung befüllter Kandidat blockt (exit 2). **Noch nicht gebaut** → AGENT_MEMORY-Folgepriorität. **Wirksamkeit prüfen:** nach Bau kein neuer OBS mit vorab notiertem Kandidaten (BEWÄHRT über mehrere Erfassungen ohne Rückfall; Regression → AKTIV). Bezug: LL-S102-1, LL-S106-1, CM-S047-1, CM-S102-2 (Muster-Vorbild).
