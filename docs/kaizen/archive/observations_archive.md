...
- Entscheidung/Maßnahme: Aus der Einheit mit OBS-S107-1/-S116-1/-S120-1/-S120-2/-S121-1 herausgeloest (S124): isolierter Feldvalidierungs-Fix, der die Tracker-Architekturfrage weder braucht noch billiger macht. Befund am Bestand korrigierte den Eintrag in zwei Punkten: die drei genannten LL-Eintraege sind laengst bereinigt (LL-Bestand inkl. Archiv heute sauber), und der einzige real wirksame Rest lag in countermeasures.md (CM-S114-1 trug den Nicht-Tag tech-debt) - einer Datei, die von Hand editiert wird und die eine argparse-choices in lessons.py prinzipiell nie erreicht. Umgesetzt daher zweigleisig: kontext_tags.py liest die Tag-Tabelle aus process.md als einzige Quelle, obs_entry/lessons_entry weisen unbekannte Tags beim Anlegen ab (jeder Schreibweg, nicht nur die CLI), und retro_report.unbekannte_tags meldet den Bestand inkl. Archiv und CM-Datei in Abschnitt 2. CM-S114-1 auf Hook/Script, Doku korrigiert. Verworfen: Werteliste als Code-Konstante (zweiter Wahrheitsort neben process.md) und Bereinigung der ~19 abweichenden Kontexte in observations.md (kein Script liest den OBS-Kontext, daher heute folgenlos - als bekannte Grenze in der CM vermerkt). Maßnahme: CM-S124-1.
- Bezug: CM-S064-2
...
- Entscheidung/Maßnahme: check-adr-capture.falsche_session blockt neu erfasste ADR-IDs, deren Session nicht die laufende ist. Die S107-Entscheidung lautete "aufschieben bis zum 2. Vorkommen"; ein zweites Vorkommen ist nicht belegt - der Punkt wurde trotzdem umgesetzt, weil er als Nebenprodukt der Tracker-Arbeit fast nichts kostete (~20 Zeilen im bestehenden Hook) und die Korrektur im Schadensfall teuer ist (~7 Referenzen). Verworfen: ein decisions.py add mit korrekter Nummernvergabe. Die Messung ergab 3 neue ADRs in 11 Sessions - eine CLI amortisiert sich dafuer schwach, und sie faenge Hand-Edits gar nicht, die bei ADR der Normalweg sind (Strategie "Edit von Hand, Hook blockt hinterher"). Geprueft werden nur NEU hinzukommende Eintraege; ohne verlaesslich bestimmbare Session wird nicht geprueft, weil ein ratender Blocker irgendwann das Richtige blockt. Escape ueber den bestehenden adr-ok-Marker. Selbstnachweis am echten Repo gefuehrt: ADR-S120-9 wird geblockt, ADR-S124-9 nicht. Maßnahme: CM-S124-2 (c).
- Bezug: LL-S106-2
...
- Entscheidung/Maßnahme: Vorschau im Freigabedialog: tracker_preview.vorschau baut aus Kommandozeile plus aktuellem Dateiinhalt einen Vorher/Nachher-Block, den check-bash-permission.py als permissionDecisionReason mitgibt - bei add eine Feldauflistung (kein Diff, es gibt kein Vorher), bei set ein farbiger Diff, bei remove der vollstaendige Eintrag, weil dieser Fall irreversibel ist. Der Kanal wurde vorher empirisch sondiert statt angenommen: Er traegt Zeilenumbrueche, Einrueckung, Unicode-Rahmen und ANSI-Styling, aber kein Markdown, und kuerzt nicht. Vorpraegung war "diff-maessig erkennbar"; die Ruecksprache ergab genauer: beim Anlegen genuegt eine leserliche Darstellung, beim Aendern sollen die Unterschiede hervorgehoben sein - so umgesetzt. Verworfen: Trockenlauf des Scripts als Subprozess. Ein PreToolUse-Hook, der den vom Agenten gelieferten Befehl ausfuehrt, waere eine Luecke im Sicherungsmechanismus selbst - eine Kommando-Substitution im Argument liefe schon bei der Vorschau. Daraus folgt der zweite Teil: Da shlex Backticks als Literal behandelt, die echte Shell sie aber ausfuehrt, meldet die Vorschau erkannte Shell-Metazeichen ausdruecklich - genau der stille Korruptionspfad aus S117. Maßnahme: CM-S124-2 (b).
- Bezug: CM-S114-2
...
- Entscheidung/Maßnahme: oq.py neu: list/get/add/set/remove, auf der gemeinsamen Basis tracker_entry.py. Das Loeschen ist die tragende Operation, nicht das Anlegen - remove entfernt den Eintrag samt genau einer angrenzenden Trennlinie (sonst bleibt eine Ruine) und meldet SELBST die Fundstellen, die noch auf die ID zeigen, statt sie check-dangling-refs.py finden zu lassen, nachdem der Vorgang schon halb gelaufen ist (Schadensfall S120: Hook blockte, danach blieb der Eintrag stehen). Die Faellig-Anker-Pruefung wird aus td_anchors wiederverwendet, nicht kopiert - sonst entstuenden Eintraege, die check-oq-capture.py danach blockt. Meine erste Bewertung war falsch und wurde vom User korrigiert: Ich hatte Bestand (4 offen) mit Durchsatz verwechselt und daraus "amortisiert sich kaum" geschlossen. TD und OQ loeschen erledigte Eintraege, LL und OBS archivieren sie - die Zahlen waren nicht vergleichbar. Aus der git-Historie: 7 OQ je angelegt, 3 geloescht, und fuer diese 3 Loeschungen entstanden 2 Wegwerf-Scripte. Maßnahme: Werkzeug gebaut (Einmal-Aenderung, keine eigene CM).
- Bezug: OBS-S114-1
...
- Entscheidung/Maßnahme: Gemeinsame Basis plus fuenf duenne CLIs (Variante E2+), nicht ein Werkzeug fuer alles (E3). tracker_entry.py traegt, was open-questions.md und tech-debt.md wirklich teilen: H2-Kopfzeile, **Feld:**-Zeilen, Trenner, ID-Vergabe, Struktur-Pruefung, Aendern, Loeschen. oq_entry und td_entry sind nur noch Feldnamen plus tracker-eigene Pruefung. Bewusste Grenze: observations.md und lessons_learned.md haben ein anderes Eintragsformat und bleiben eigenstaendig - sie einzupassen hiesse, die Basis um Sonderfaelle aufzublaehen, bis sie nichts mehr vereinfacht. Abdeckungsluecken geschlossen: td.py und oq.py neu (get/list/add/set/remove), ADR-Session-Pruefung im Capture-Hook. Der User-Einwand zu E3 - fuenf --help-Aufrufe statt einem - ist per tracker.py geloest, das die Zustaendigkeiten und die gemeinsame Grammatik an einer Stelle zeigt und die Befehlsliste aus dem --help der Scripte selbst liest, statt eine driftende Kopie zu pflegen. Verworfen: Delegation (tracker.py oq remove …). Sie liefe an den WRITE_ACCESS-Mustern vorbei, die auf oq.py remove passen; jedes Muster muesste beide Formen kennen, ein vergessenes waere ein stiller Freibrief auf ein versioniertes Dokument. Ebenfalls verworfen: Vereinheitlichung der Eintragsformate selbst - das waere eine Migration aller Bestandsdateien ohne Gegenwert. Die Frage aus dem Eintrag, ob Vereinheitlichung die Trennschaerfe aufweicht, ist damit beantwortet: Die Taxonomie bleibt unberuehrt, weil jeder Tracker sein eigenes Werkzeug mit eigenen Pflichtfeldern behaelt. Maßnahme: Werkzeuge gebaut; die Poka-Yokes darin unter CM-S124-2.
...
- Entscheidung/Maßnahme: Struktur-Invariante statt Vollstaendigkeitspruefung: pruefe_wohlgeformt laeuft VOR und NACH jedem Umbau (obs_entry, tracker_entry). Vorher, weil ein verrutschtes Feld jeden weiteren Schreibzugriff zum Bruch macht; nachher, weil ein Wert, der wie eine Feldzeile aussieht, die Struktur kapern koennte. Unterschieden wird "fehlt" (Feldname kommt gar nicht vor - bei Alteintraegen legitim) von "verrutscht" (kommt vor, aber nicht am Zeilenanfang). Verworfen wurde die Vollstaendigkeitspruefung: Sie haette jeden Eintrag aus der Zeit vor einem spaeter eingefuehrten Pflichtfeld unaenderbar gemacht. Am Bestand verifiziert: alle 111 OBS-Eintraege bestehen die Pruefung. Reichweite-Frage aus dem Eintrag beantwortet: gemeinsam in tracker_entry fuer TD/OQ, zusaetzlich in obs_entry (anderes Eintragsformat). lessons.py braucht keinen - es hat keinen Aenderungspfad. Maßnahme: CM-S124-2 (a).
- Bezug: LL-S121-1
...
- Zusammen-erledigen: keiner
- Entscheidung/Maßnahme: Platzhalter statt Kopie (Variante V3, User-Entscheid S124). Die AGENT_MEMORY-Zeile eines TD-Punkts traegt nur noch ID und Done-Kriterium; session-agenda.py loest Titel und Faelligkeit beim Rendern aus tech-debt.md auf (td_entry.memory_aufloesen). Damit existiert der Titel nur einmal und kann nicht driften. Die drei Dinge, die der Eintrag als schuetzenswert nennt, bleiben erhalten: die Rangfolge (Position in der Liste, handgepflegt), das Done-Kriterium (im TD-Format kein Feld, lebt weiter nur hier) und die Mischung mit Punkten aus anderen Quellen (die bleiben voll ausgeschrieben). td.py add/set/remove erzeugt und entfernt den Punkt, sodass auch das Raeumen zweier Dateien entfaellt. Der bestehende Gate check-td-capture bleibt wirksam - am Bestand gegengeprueft: mit Platzhalter keine Beanstandung, ohne Punkt blockt er weiterhin. Verworfen: (1) mechanisierte Doppelpflege - loest das Raeumen, laesst aber den Titel-Drift bestehen; ich hatte sie zunaechst empfohlen, was der User zu Recht als Sunk-Cost-Argument hinterfragt hat. (2) Vollstaendiges Generieren aus tech-debt.md - haette ein neues TD-Pflichtfeld fuer Done gebraucht (Formataenderung plus Migration von 28 Eintraegen), einen Ueberschreibungs-Mechanismus fuer die quellenuebergreifende Rangfolge, und den Gate gegenstandslos gemacht; die Pflege waere gewandert statt verschwunden. Bekannter Preis von V3: Der Punkt traegt den beschreibenden TD-Titel statt eines handlungsorientierten; die Handlungsrichtung gehoert seither ins Done-Kriterium. Ein optionaler Titel-Override waere die Tuer zurueck zur Duplikation und wurde bewusst nicht gebaut. Maßnahme: Format umgestellt und im AGENT_MEMORY-Header dokumentiert (Einmal-Aenderung; die Werkzeug-Poka-Yokes darin unter CM-S124-2).
- Bezug: OBS-S116-2
