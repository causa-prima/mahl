...
kritische-regeln:
  ...
  - Neues Paket → erst Allowlist erweitern + adr.md → dann installieren
...
3. Die Entscheidung ist in `docs/history/adr.md` begründet
...
Erst nach expliziter Freigabe: Eintrag in Allowlist + Begründung in `docs/history/adr.md` + Installation.

## Allowlist

> Freigegeben Session 044 (2026-04-01). Begründungen in `docs/history/adr.md`.
...
Hooks blockieren Änderungen an `Client/package.json`, `**/*.csproj` und `docs/reference/dependencies.md` durch den Agenten vollständig. Alle drei Dateien müssen vom User manuell bearbeitet werden.
