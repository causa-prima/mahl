...
**Behebung/Trigger:** (b) reist mit der Theme-Foundation aus TD-S083-2 – gleiche Datei, gleiche visuelle Baseline, dort bereits terminiert. (a) und (c) sind reine Refactorings **ohne beobachtbare Verhaltensänderung** und brauchen deshalb keinen Auslöser und kein Szenario: bei grünen Tests mitnehmen, sobald die Datei ohnehin angefasst wird – spätestens mit (b).
...
**Priorität:** Hoch – kein Politur-Posten: die Accessibility-Anforderung gilt heute und wird heute verletzt
...
**Behebung/Trigger:** Auslöser ist die Erweiterung von `useResultQuery` um einen Query-Zustand (ADR-S083-2, gemeinsam mit TD-S083-1, TD-S090-2 und TD-S101-1). **Vorher nicht umsetzbar:** `useResultQuery` liefert ausschließlich `TData | undefined` und exponiert keinerlei Lade-/Fehlerzustand – die Seite *kann* nicht wissen, ob der initiale GET gesettled ist. Danach: Speichern sperren, solange die Ingredients-Query nicht gesettled ist. **Achtung:** Das vorhandene `disabled={isPending}` löst das nicht – dessen `isPending` stammt aus `useCreateIngredientWithReactivation` und ist der Zustand der *POST-Mutation*, greift also erst, nachdem der POST bereits feuerte.
...
**Behebung/Trigger:** Nominale Brands nach ADR-S112-4 (Vergabe an der API-Grenze, keine Regelprüfung). Auslöser: die nächste Frontend-Story, die eigene Domänentypen einführt – dann entsteht das Muster ohnehin und wird für die bestehenden beiden Typen mitgezogen.
...
**Behebung/Trigger:** Als eigene Aufgabe **vor dem Beginn der nächsten Story** erledigen – nicht an einen Code-Trigger gehängt, weil `tech-debt.md` nur situativ gelesen wird und ein Trigger allein hier den Eintrag seit S089 nicht bewegt hat. Terminiert wird er deshalb über die Prioritätenliste in `docs/AGENT_MEMORY.md`, die am Session-Start gelesen wird. Schritte: Coverage-Engine für den tatsächlichen MTP-1.9-Stack ermitteln, aufsetzen, dann `collect_coverage` in `dotnet-test.py` reaktivieren und 100% Branch-Coverage verifizieren. **Nicht erneut probieren** (bereits gescheitert, grenzt den Lösungsraum ein): `Microsoft.Testing.Extensions.CodeCoverage` 18.3.2 und `coverlet.MTP` 8.0.1/10.0.1 → `TypeLoadException`; CodeCoverage 17.14.2 lief, scheiterte am `--coverage-settings`-Format. Diese Versuche liefen gegen MTP 2.x – ob sie gegen den heutigen 1.9er-Stack anders ausgehen, ist ungeprüft.
...
**Behebung/Trigger:** `string?`-Properties + bewusste Null-Behandlung in `ToDomain`, oder ein einheitlicher 4xx→`{"errors"}`-Mapper. Auslöser: sobald ein **anderer Client als unser Frontend** das API nutzt (die Lücke ist bis dahin strukturell unerreichbar) – spätestens mit einem öffentlich erreichbaren API ab MVP.
...
**Behebung/Trigger:** Gemeinsam mit TD-S090-2 beim resilience-`QueryCache.onError`-Setup: erst dann ist `throwOnError` möglich, erst dann hat die Zustands-Union ausgeübte Zweige – vorher entstünden Survivor und Suppressions außerhalb des treibenden Szenarios.
...
**Behebung/Trigger:** Explizites app-weites `MaxRequestBodySize` in `Program.cs` (statt Verlass auf den impliziten Framework-Default) – vor MVP-Auth, spätestens sobald ein Endpoint authentifizierte oder größere Payloads annimmt.

---

## TD-S106-1 — Backend: kein globaler Exception-Handler / ProblemDetails-Fallback
**Priorität:** Niedrig – Anforderung greift erst ab MVP (`docs/process/nfr.md`, Sektion Security: „Fehlerantworten enthalten keine technischen Details"); der konkrete run-10-500-Pfad (malformed If-Match) ist bereits gefixt (ADR-S106-2)
...
**Reichweite präzisiert (an learn.microsoft.com verifiziert, Kommentar an der Stelle in `Server/Program.cs`):** Ohne Handler liefert Kestrel in Production einen **500 ohne Response-Body** – dort leakt nichts. Der Stack-Trace-Leak über die Developer-Exception-Page setzt `ASPNETCORE_ENVIRONMENT=Development` voraus, was ausschließlich `Server/Properties/launchSettings.json` beim lokalen Start setzt, nie ein Deployment. Zu korrigieren ist außerdem die frühere Begründung: Der NFR „keine Stack-Traces für User" (`nfr.md`, Reliability) ist eine **Frontend**-Anforderung – die UI zeigt nichts Technisches an – und trägt diesen Backend-Eintrag nicht. Die tragende Anforderung ist der separate Security-NFR („Fehlerantworten enthalten keine technischen Details"), und der gilt **ab MVP**.
**Behebung/Trigger:** Ab MVP umsetzen – Entscheidung, Begründung, Verworfenes und offene Punkte stehen in **ADR-S112-1** (unbedingte Registrierung von `UseExceptionHandler`, damit der Fehlerpfad keinen Umgebungs-Zweig mehr hat). Prüfbar als Backend-Integrationstest (`WebApplicationFactory`), nicht per E2E – die Eigenschaft ist an der Oberfläche nicht beobachtbar. **Vorbedingung:** Ein Logging-Konzept muss geklärt sein; der Stack-Trace soll ins Log wandern, ein gestaltetes Log existiert aber noch nicht (keine `Logging`-Sektion, keine `ILogger`-Nutzung). Zieht `TD-S084-1(b)` (try/finally in der ETag-Middleware) unmittelbar nach sich.
...
**Behebung/Trigger:** Zusätzliche Schließen-Action (`CloseIcon`) neben „Rückgängig" – ein expliziter Button ist eine bewusste Geste und kollidiert nicht mit dem clickaway-Guard, der nur beiläufige Klicks abfängt. Weil es sichtbares Nutzerverhalten ist, muss ein Gherkin-Szenario davor liegen; es ist in der Prioritätenliste von `docs/AGENT_MEMORY.md` als aufzunehmen vermerkt. Ein rein reaktiver Auslöser („der nächste Lauf, der den Toast ohnehin verändert") ist hier ausdrücklich **nicht** ausreichend – er wurde bereits einmal übersehen, während der Toast-Bereich verändert wurde (LL-S111-2).
...
**Behebung/Trigger:** (a) und (b) gemeinsam mit TD-S108-1 beheben und dessen Auslöser teilen – sobald der Delete/Restore-Pfad eine echte Fehler-Union bekommt, ist der Fehler-Zweig ohnehin vorhanden und `deletingId` dort mit zurückzusetzen. (c) und (d) brauchen je ein Gherkin-Szenario, bevor sie umgesetzt werden dürfen: für (c) „mehrere gleichzeitige Löschvorgänge", für (d) „Rückgängig ist während des laufenden Wiederherstellens gesperrt". Eine Umstellung von `deletingId` auf `ReadonlySet<string>` **vorab** erzeugt einen Zweig, den kein Szenario ausübt → Stryker-Survivor → Suppression außerhalb des treibenden Szenarios, genau die Konstellation, die ADR-S083-2 vermeiden will. Beide Szenarien sind in der Prioritätenliste von `docs/AGENT_MEMORY.md` als aufzunehmen vermerkt; die Umsetzung von (d) ist dann analog zu `deletingId` ein Pending-Flag im selben Hook.

---

## TD-S112-1 — Konfigurations-Parität E2E↔Produktion und Umgebungs-Allow-Liste nicht umgesetzt
**Priorität:** Mittel – ab MVP, gemeinsam mit TD-S106-1
...
**Behebung/Trigger:** Ab MVP zusammen mit dem unbedingten Exception-Handler (TD-S106-1) umsetzen – (c) hängt an (b), und der Wächter ist ohne den Handler gegenstandslos. Beim Umsetzen verifizieren, dass der Startup-Guard die werkzeuggebauten Hosts (`dotnet ef`, `WebApplicationFactory`) nicht bricht.

---

## TD-S112-2 — Observability-Anforderung ohne Umsetzung: Backend loggt nirgends aktiv
**Priorität:** Mittel – gemeinsam mit TD-S106-1, ab MVP
...
**Behebung/Trigger:** Mit TD-S106-1 zusammen: empirisch prüfen, ob eine unbehandelte Exception tatsächlich mit Stack-Trace auf stdout landet, und die Zusicherung mit einem Test festnageln. Nur falls die Prüfung eine Lücke zeigt, explizite Logging-Konfiguration ergänzen – nicht vorsorglich.
