...
**Problem:** `IngredientValuesDto(string Name, string DefaultUnit)` (bis run-11 `CreateIngredientDto`) ist non-nullable. Lässt ein Client das `name`-Property **ganz weg** (statt `""`), kann ASP.NET Minimal API je nach STJ-Konfiguration `null` binden (Warnung) oder **400** vor dem Handler werfen — nicht das vertragliche **422** mit `{"errors":{…}}`. Der Client parst in `toIngredientResult` nur `status === 422` als Fehler; ein 400 würde als `Ingredient` interpretiert → stiller Fehlzustand. Aktuell unerreichbar (das Szenario sendet stets `name: ""`).
**Reichweite seit run-11 verdoppelt:** Derselbe DTO wird jetzt von **zwei** Endpoints per Body gebunden – `POST /api/ingredients` und `POST /{id}/restore` (ADR-S111-1). Das Problem trifft damit auch den Restore-Pfad, und dort ist die Client-Seite noch dünner: `toRestoreOutcome` wertet ausschließlich den 409 aus (s. TD-S108-1(b)), ein 400 liefe als Erfolg durch.
...
**Teilweise überholt durch run-11:** `restoreIngredient` wertet den Status jetzt aus – aber nur den `409` (`toRestoreOutcome`, ADR-S111-1). Alles andere fällt weiterhin in den Erfolgszweig und wird als `Ingredient` geparst; bei einem `404` entstünde ein `Restored`-Outcome mit einem ProblemDetails-Objekt als vermeintlicher Zutat, ohne dass irgendwo ein Fehler sichtbar würde. Verschärfend: Seit run-11 kann der Restore-Endpoint überhaupt erst mit `422` antworten (Namenskollision mit einer anderen Zeile, s. ADR-S111-1-Addendum) – ein Statuscode, den der Client nicht kennt. Über die aktuelle UI nicht auslösbar, weil pro LOWER-Name nur eine Zeile existieren kann; der Pfad ist aber jetzt vorhanden. `deleteIngredient` ist unverändert ohne jeden Status-Check.
...
**Ausgeweitet durch run-11 (zweiter Toast, längere Dauer):** Der neue `ReactivationConflictToast` (ADR-S111-3) hat dasselbe Problem in schärferer Form. Er schließt zwar bei clickaway (kein Aktions-Button, der geschützt werden müsste), steht aber **10 Sekunden** statt 6 – bewusst so entkoppelt, weil der Text rund zehnmal so lang ist. Genau das Argument, das die längere Dauer rechtfertigt, verschärft die Lücke: MUIs Pause-bei-Hover greift auf Touch nicht, und auf dem primären Zielgerät bleibt die Meldung damit am längsten stehen, ohne dass der Nutzer sie aktiv wegräumen kann.
**Behebung/Trigger:** Zusätzliche Schließen-Action (`CloseIcon`) neben „Rückgängig" – ein expliziter Button ist eine bewusste Geste und kollidiert nicht mit dem clickaway-Guard, der nur beiläufige Klicks abfängt. Da es sichtbares Nutzerverhalten ist, gehört ein Gherkin-Szenario davor (nicht wie in run-8 nachträglich). **Trigger neu gefasst (run-11):** Die bisherige Formulierung „der nächste Lauf, der den Toast ohnehin verändert" hat sich als untauglich erwiesen – run-11 hat den Toast-Bereich verändert (Undo-Toast wird bei Reaktivierung verworfen, zweiter Toast eingeführt), ohne dass der Trigger jemandem auffiel; ein rein reaktiver Trigger wird beim Implementieren schlicht nicht gelesen. Stattdessen **aktiv im `gherkin-workshop` der nächsten Zutaten-Story ein Szenario dafür aufnehmen** – dann liegt das geforderte Szenario vor der Implementierung vor, statt auf einen Lauf zu warten, der zufällig vorbeikommt.
...
(d) **Kein Pending-Guard am „Rückgängig"-Button (run-11).** `deletingId` sperrt die Zeile während des DELETE; für den umgekehrten Weg gibt es nichts Vergleichbares – der „Rückgängig"-Button bleibt klickbar, während sein Restore noch läuft. Zwei schnelle Klicks lösen zwei parallele Restores derselben Zeile aus. Serverseitig ist das seit run-11 sauber abgefangen (identische Werte → `200`, der Verlierer eines echten Races → `200`/`409` statt `500`, s. ADR-S111-1-Addendum), es entsteht also kein Datenschaden – die Lücke ist rein UI-seitig: eine laufende Aktion ohne sichtbare Rückmeldung, erneut auslösbar. Im Review von run-11 bewusst **nicht** mitbehoben: Eine Button-Sperre ist sichtbares Nutzerverhalten und braucht ein eigenes Gherkin-Szenario (run-9 hat für den analogen Löschen-Button genau eines), das hier nicht vorlag. Quelle: functional-correctness-auditor (Review run-11), Scope-Entscheidung Orchestrator.
Keine der vier Lücken ist heute durch ein Szenario beobachtbar: run-9 ist ein Singleton-Happy-Path („nur Mehl existiert"), Fehlerpfade sind ihm bewusst entzogen; für (d) fehlt das Szenario, das die Sperre überhaupt fordert.
**Behebung/Trigger:** (a) und (b) gemeinsam mit TD-S108-1 beheben – sobald der Delete/Restore-Pfad eine echte Fehler-Union bekommt, ist der Fehler-Zweig ohnehin vorhanden und `deletingId` dort mit zurückzusetzen. (c) erst mit dem ersten Szenario, das mehrere gleichzeitige Löschvorgänge fordert: eine Umstellung auf `ReadonlySet<string>` **vorab** erzeugt einen Zweig, den kein Szenario ausübt → Stryker-Survivor → Suppression außerhalb des treibenden Szenarios, genau die Konstellation, die ADR-S083-2 vermeiden will. (d) braucht zuerst ein Gherkin-Szenario („Rückgängig ist während des laufenden Wiederherstellens gesperrt") – im `gherkin-workshop` der nächsten Zutaten-Story aufnehmen; die Umsetzung ist dann analog zu `deletingId` ein Pending-Flag im selben Hook.
