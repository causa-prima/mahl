...
- **US-904 weiter:**
  - **Zutaten-Dialog: Formular-UX-Baseline (Prinzip 8) vervollständigen** → **TD-S094-1** (Fokus-aufs-Fehlerfeld: passend bei run-4 „Einheit-Validierung"; Enter-Submit: passend bei run-2, aus run-1 bewusst herausgehalten – framework-geliefert → Review, kein Szenario) + **TD-S077-1** (Escape: passend bei run-2 „Dialog-Verhalten"); jeweils mit dem betreffenden Lauf miterledigen.
