...
public readonly record struct NonEmptyTrimmedString
{
    ...
    public static OneOf<NonEmptyTrimmedString, Error<string>> Create(string value)
    {
        var trimmed = (TrimmedString)value;
        return Validate(trimmed).Match<OneOf<NonEmptyTrimmedString, Error<string>>>(
            success => new NonEmptyTrimmedString(trimmed),
            error => error
            );
    }
