...
internal abstract record IngredientValidationError
{
    ...
    // ADR-S004-1 (Addendum S105): aktives Duplikat, case-insensitiv (ADR-S051-3). Value-Träger-Subtyp – die
    // Meldung braucht den getrimmten EINGEGEBENEN Namen (nicht den gespeicherten), daher Payload statt payloadlos.
    ...
    public static IngredientValidationError NameDuplicate(string enteredName) => new NameDuplicateCase(enteredName);

    [System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage] // ADR-S040-1: structurally unreachable _-arm
    public T Match<T>(
        Func<T> onNameEmpty, Func<T> onNameTooLong, Func<T> onUnitEmpty, Func<T> onUnitTooLong,
        Func<string, T> onNameDuplicate) => this switch
    {
        NameEmptyCase => onNameEmpty(),
        NameTooLongCase => onNameTooLong(),
        UnitEmptyCase => onUnitEmpty(),
        UnitTooLongCase => onUnitTooLong(),
        NameDuplicateCase c => onNameDuplicate(c.EnteredName),
