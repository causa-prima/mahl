...
ruff_run = import_module("ruff-run")
...
_FUNDE = (".claude/scripts/foo.py:12:5: F401 [*] `os` imported but unused\n"
          ".claude/scripts/bar.py:3:1: B023 Function definition does not bind loop variable\n"
          "Found 2 errors.\n")
...
def test_der_linter_laeuft_ueber_die_konfigurierten_pfade(monkeypatch):
    ...
    monkeypatch.setattr(ruff_run, "_laufe", _merke)
    ruff_run.main([])
    assert any(".claude/scripts" in teil for teil in gesehen["befehl"])
    assert any(".claude/hooks" in teil for teil in gesehen["befehl"])
