...
from checks import ruff_lint as rl
...
# --- Relevanz ----------------------------------------------------------------
def test_prueft_python_unter_scripts_und_hooks():
    assert rl._is_watched(".claude/scripts/qa-check.py")
    assert rl._is_watched(".claude/hooks/checks/rop.py")
...
def test_saubere_datei_erzeugt_keine_warnung(monkeypatch):
    ...
    assert rl.check(make_input(".claude/scripts/qa-check.py", "x = 1\n")) == []


def test_fundstellen_erscheinen_in_der_warnung(monkeypatch):
    monkeypatch.setattr(rl, "_ruff_vorhanden", lambda: True)
    monkeypatch.setattr(rl, "_laufe", lambda *_a: _Lauf(
        ".claude/scripts/qa-check.py:12:5: F401 `os` imported but unused\n"
        "Found 1 error.\n", 1))
    warnungen = rl.check(make_input(".claude/scripts/qa-check.py", "x = 1\n"))
    assert len(warnungen) == 1
    assert "F401" in warnungen[0]
    assert "ruff-run.py" in warnungen[0], "der Weg zum vollen Lauf fehlt"
...
def test_geprueft_wird_die_geaenderte_datei(monkeypatch):
    ...
    rl.check(make_input(".claude/scripts/doc.py", "x = 1\n"))
