...
# Die Hook-Meldungen in `.claude/hooks/checks/*.py` verweisen auf genau die Guideline-
...
def test_gewoehnliche_testdatei_wird_geprueft():
    ...
    assert anchors.wird_geprueft(".claude/hooks/tests/test_primitives.py")
...
def test_pruefwerkzeug_test_ist_nur_musterweise_ausgenommen():
    ...
    assert anchors.wird_geprueft(".claude/hooks/tests/test_ordinale.py")
    assert "ordinal" in anchors.ausnahmen_fuer(".claude/hooks/tests/test_ordinale.py")
    assert "verweis" in anchors.ausnahmen_fuer(".claude/hooks/tests/test_anchors.py")
...
def test_py_wird_gescannt(tmp_path):
    ...
    (tmp_path / ".claude" / "hooks" / "h.py").write_text("x = 1\n", encoding="utf-8")
    assert ".claude/hooks/h.py" in anchors.lies_bestand(tmp_path)
...
def test_klartext_pfad_wird_repo_relativ_gelesen():
    ...
    Markdown, meldete der Prüfer jede Meldung aus `.claude/hooks/checks/` als falschen Pfad."""
    bestand = dict(BESTAND)
    bestand[".claude/hooks/checks/c.py"] = 'm = "docs/guidelines/typescript.md#CGT-result"\n'
    assert anchors.falsche_pfade(bestand) == []
...
def test_falscher_klartext_pfad_wird_gemeldet():
    ...
    bestand = dict(BESTAND)
    bestand[".claude/hooks/checks/c.py"] = 'm = "docs/process/nfr.md#CGT-result"\n'
    assert anchors.falsche_pfade(bestand) == [
        (".claude/hooks/checks/c.py", 1, "CGT-result",
         "docs/process/nfr.md", "docs/guidelines/typescript.md")]
