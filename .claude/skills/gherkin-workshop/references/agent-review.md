...
- `docs/guidelines/coding-guideline-ux.md` – Interaction-Design-Prinzipien („Sichtbares Feedback", „Fehlermeldungen als Hilfe", „Leerer Zustand" sind für Gherkin relevant)

Feature-Entwurf: [vollständiger Entwurf aus der Synthese, verbatim]
User Story + Akzeptanzkriterien: [aus dem Kontext-Laden, verbatim – nicht paraphrasiert; Paraphrasen
  können implizite Anforderungen verlieren]
Constraints aus adr.md: [aus dem Kontext-Laden]
Resilience-Entscheidung aus der Regelentdeckung: [entweder „story-spezifisches Verhalten: <Beschreibung>"
  oder „allgemeine Behandlung gilt, kein story-spezifisches Szenario nötig"]
Draft-Saving-Entscheidung aus der Regelentdeckung: [entweder „nicht-trivialer Eingabeaufwand: Szenario
...
- Leerer Zustand ohne Erklärungstext und nächste Aktion („Leerer Zustand" – docs/guidelines/coding-guideline-ux.md): Then beschreibt nur "leere Liste" statt was der Nutzer konkret sieht
...
- Mutierende Operation mit Server-Wartezeit ohne Ladezustand-Szenario („Sichtbares Feedback"): kein Szenario das beschreibt was der Nutzer während des Wartens sieht – nur wenn die Operation typischerweise spürbar lange dauert
- UI-Verhaltensaspekte ohne Szenario: Wenn die Story ein Formular oder Dialog hat und keines der vorhandenen Szenarien beschreibt (a) was nach Erfolg mit dem Dialog passiert, (b) einen Abbrechen-Pfad, (c) den initialen Feld-Zustand beim Öffnen, oder (d) Loading/Disabled-States — und diese Aspekte wurden in der Regelentdeckung als relevant markiert: als MEDIUM flaggen mit konkretem Hinweis welcher Aspekt fehlt
