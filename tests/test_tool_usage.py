...
def test_plain_run_is_counted_unfiltered():
    assert tu.classify_line(f"{STAMP}python3 -m prozesscode.vitest-run") == ("2026-07", "vitest-run", False)


def test_trailing_filter_is_counted():
    line = f"{STAMP}python3 -m prozesscode.qa-check | tail -5"
...
def test_filter_before_the_wrapper_does_not_count():
    ...
    line = f"{STAMP}grep -r foo . | python3 -m prozesscode.qa-check"
...
def test_reading_a_wrapper_file_is_not_a_run():
    ...
    assert tu.classify_line(f"{STAMP}grep -n collect_coverage prozesscode/dotnet-test.py") is None
...
def test_non_wrapper_commands_are_ignored():
    assert tu.classify_line(f"{STAMP}python3 -m prozesscode.decisions list") is None
    assert tu.classify_line(f"{STAMP}git status") is None


def test_absolute_wrapper_path_is_recognized():
    line = f"{STAMP}python3 /home/kieritz/repos/mahl/prozesscode/eslint-run.py | grep error"
...
def test_analysis_scripts_are_deliberately_not_wrappers():
    ...
    assert tu.classify_line(f"{STAMP}python3 -m prozesscode.read-breakdown | head") is None
...
def test_measure_counts_runs_and_quote(tmp_path):
    ...
    log.write_text(
        f"{STAMP}python3 -m prozesscode.vitest-run\n"
        f"{STAMP}python3 -m prozesscode.vitest-run | tail -3\n"
        "[2026-06-01 09:00:00] python3 -m prozesscode.qa-check | grep OK\n"
        f"{STAMP}git status\n",
        encoding="utf-8",
    )
...
def test_line_date_yields_the_full_day():
    assert tu.line_date(f"{STAMP}python3 -m prozesscode.vitest-run") == "2026-07-15"
...
def test_since_skips_earlier_runs(tmp_path):
    log = tmp_path / "allowed-commands.log"
    log.write_text(
        "[2026-07-28 09:00:00] python3 -m prozesscode.vitest-run | tail -3\n"
        "[2026-07-30 09:00:00] python3 -m prozesscode.vitest-run | tail -3\n"
        "[2026-08-01 09:00:00] python3 -m prozesscode.qa-check\n",
        encoding="utf-8",
    )
