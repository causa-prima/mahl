...
from prozesscode.hooks.checks import tooling_tests as tt
from conftest import make_input


# --- Relevanz: welche Datei löst einen Lauf aus? -----------------------------
def test_watches_scripts_hooks_and_tests():
    assert tt._is_watched("prozesscode/qa-check.py")
    assert tt._is_watched("prozesscode/hooks/check-ref-direction.py")
    assert tt._is_watched("/home/x/repo/prozesscode/hooks/checks/rop.py")
    assert tt._is_watched("tests/test_tooling_tests.py")
...
_STDOUT = """\
/home/x/repo/tests/test_qa_check.py:37: assert None == 100.0
/home/x/repo/tests/test_obs_drain.py:12: assert 3 == 4
...
def test_format_caps_long_failure_lists():
    lines = "\n".join(
        f"/repo/tests/test_x.py:{i}: assert {i} == 0" for i in range(25)
    )
