...
def test_der_mutanten_baum_wird_nicht_gescannt(monkeypatch, tmp_path):
    """`mutants/` ist eine Repo-Kopie von mutmut – Verweise darin sind Kopien, keine Verweise.

    Ohne den Ausschluss ließe sich nach einem Mutationslauf kein erledigter Eintrag mehr
    löschen: Die Kopie enthält jeden Verweis ein zweites Mal, und der Hook blockt darauf.
    """
    monkeypatch.setattr(hook, "_REPO_ROOT", tmp_path)
    d = tmp_path / "mutants" / "docs"
    d.mkdir(parents=True)
    (d / "kopie.md").write_text("Siehe TD-S001-1\n", encoding="utf-8")
...
def test_real_reference_outside_tests_still_blocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(hook, "_REPO_ROOT", tmp_path)
    paket = tmp_path / "prozesscode"
    paket.mkdir(parents=True)
    (paket / "dotnet-test.py").write_text("# siehe TD-S001-1\n", encoding="utf-8")
