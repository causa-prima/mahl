...
def test_redirect_detection() -> int:
    ...
    cases = [
        ...
        ("echo text >> prozesscode/hooks/check-bash-permission.py", True, "Append auf Hook"),
...
def test_allow_patterns() -> int:
    ...
    allow_cases = [
        ...
        # python3 -m pytest auf tests/
        ("python3 -m pytest tests/ -p no:cacheprovider -s -q", "pytest tests/"),
        ("python3 -m pytest tests/test_hook_io.py -q", "pytest einzelne Testdatei"),
        # Eigenständige Runner in tests/ – test-bash-permission.py bringt sein eigenes
        # main() mit und wird von der Fehlermeldung genau so zum Nachfahren angeboten.
        ("python3 tests/test-bash-permission.py", "eigenständiger Test-Runner"),
        # Projekt-Scripts
        ("python3 -m prozesscode.dotnet-test --filter TestName", "dotnet-test.py"),
        ("python3 -m prozesscode.dotnet-stryker --mutate Domain/Foo.cs --verbose", "dotnet-stryker.py"),
        ...
        ("chmod +x prozesscode/hooks/new-hook.sh", "chmod +x"),
        ...
        # Python: nur .claude/
        ("python3 -m prozesscode.stryker-summary", "python3 prozesscode/"),
        ("python3 -m prozesscode.jenga_score", "jenga_score.py"),
        ("python3 -m prozesscode.jenga_score --file docs/kaizen/lessons_learned.md", "jenga_score.py mit --file"),
        ...
        ("python3 -m prozesscode.vitest-run", "vitest-run.py"),
        ("python3 -m prozesscode.vitest-run --filter IngredientsPage", "vitest-run.py --filter"),
        ("python3 -m prozesscode.vitest-run --verbose", "vitest-run.py --verbose"),
        ("python3 -m prozesscode.playwright-test", "playwright-test.py"),
        ("python3 -m prozesscode.playwright-test --filter ingredients", "playwright-test.py --filter"),
        ("python3 -m prozesscode.stryker-frontend", "stryker-frontend.py"),
        ("python3 -m prozesscode.stryker-frontend --mutate src/pages/IngredientsPage.tsx", "stryker-frontend.py --mutate"),
        ("python3 -m prozesscode.stryker-frontend --verbose", "stryker-frontend.py --verbose"),
        ...
        ("cd Client", "cd solo"),
        ("cd prozesscode/hooks && ls -la", "cd && ls"),
...
def test_one_time_marker() -> int:
    ...
    allow_cases = [
        ...
        ('python3 -m prozesscode.x # --allow-once', "erlaubtes Script + Marker → allow"),
    ]
    for command, desc in allow_cases:
        if not assert_decision(command, "allow", f"{desc}: {command}"):
            failures += 1
...
def test_write_access_scripts() -> int:
    ...
    ask_cases = [
        ('python3 -m prozesscode.obs add --titel "X" --beobachtung "Y"',
         "obs.py add → ask"),
        ('python3 -m prozesscode.obs set OBS-S114-1 --status "UMGESETZT (S114)"',
         "obs.py set → ask"),
        ('python3 -m prozesscode.lessons add --titel "X" --regel "Y"',
         "lessons.py add → ask"),
        # Auch als Teil einer Verkettung darf der Text nicht ungesehen durchgehen.
        ('git status && python3 -m prozesscode.obs add --titel "X"',
        ...
        ('python3 -m prozesscode.obs add --titel "Warum --help nervt" --beobachtung "Y"',
    ...
    allow_cases = [
        ('python3 -m prozesscode.obs get OBS-S112-7', "obs.py get bleibt allow (read-only)"),
        ('python3 -m prozesscode.lessons get LL-S113-1',
         "lessons.py get bleibt allow (read-only)"),
        # Rein mechanisches Verschieben ohne neuen Text – bewusst weiter ohne Freigabe.
        ('python3 -m prozesscode.obs-archive', "obs-archive.py bleibt allow"),
        ('python3 -m prozesscode.obs-drain', "obs-drain.py bleibt allow"),
        # Kein Fehlalarm auf gleichnamige, aber andere Scripte.
        ('python3 -m prozesscode.next_run --open', "next_run als Paketmodul bleibt allow"),
        ...
        ('python3 -m prozesscode.obs add --help', "obs.py add --help bleibt allow"),
        ('python3 -m prozesscode.td set -h', "td.py set -h bleibt allow"),
        ('python3 -m prozesscode.lessons add --help', "lessons.py add --help bleibt allow"),
    ]
...
def test_edge_cases() -> int:
    ...
    cases = [
        ...
        ("chmod +x prozesscode/hooks/script.sh", "allow", "chmod +x erlaubt"),
...
def test_path_normalization() -> int:
    ...
    norm_cases = [
        ...
        (f"python3 {R}/tests/test-bash-permission.py",
         "python3 tests/test-bash-permission.py", True, "python3 absoluter Repo-Pfad"),
    ...
    flip_cases = [
        (f"cat {R}/docs/AGENT_MEMORY.md", "allow",
         "absoluter Repo-Pfad zu Projektdatei → allow (vorher deny)"),
        (f"python3 {R}/tests/test-bash-permission.py", "allow",
         "absoluter Repo-Pfad zu Test-Runner → allow"),
    ]
    ...
    if not assert_decision(f"python3 {R}/tests/x.py # --allow-once", "allow",
    ...
    changed_cmd = f"python3 {R}/tests/test-bash-permission.py"
    out = build_allow_output(changed_cmd)
    if (out.get("updatedInput", {}).get("command") == "python3 tests/test-bash-permission.py"
...
def test_wrapper_filter_strip() -> int:
    ...
    cases = [
        ("python3 -m prozesscode.vitest-run | tail -50",
         "python3 -m prozesscode.vitest-run", True, "tail nach Wrapper"),
        ("python3 -m prozesscode.qa-check --layer frontend 2>&1 | grep OK",
         "python3 -m prozesscode.qa-check --layer frontend 2>&1", True, "grep, 2>&1 bleibt"),
        ("python3 -m prozesscode.dotnet-test | head -3 | grep x",
         "python3 -m prozesscode.dotnet-test", True, "mehrere Filter hintereinander"),
        # Filter VOR dem Wrapper filtert dessen Ausgabe nicht – unberührt lassen.
        ("grep -r foo . | python3 -m prozesscode.qa-check --layer backend",
         "grep -r foo . | python3 -m prozesscode.qa-check --layer backend", False,
         "Filter vor dem Wrapper bleibt"),
        # Analyse-Scripte sind zum Zerschneiden gedacht (dieselbe Abgrenzung wie tool-usage.py).
        ("python3 -m prozesscode.read-breakdown | head -20",
         "python3 -m prozesscode.read-breakdown | head -20", False,
    ...
    out = build_allow_output("python3 -m prozesscode.vitest-run | tail -50")
    if (out.get("updatedInput", {}).get("command") == "python3 -m prozesscode.vitest-run"
            and out.get("permissionDecision") == "allow"
    ...
    # Beide Rewrites zusammen: absoluter Pfad UND Filter in einem Befehl. Der absolute Pfad
    # sitzt seit dem Paket-Umzug im `cd`, nicht mehr im Wrapper-Aufruf – `-m` trägt keinen.
    both = build_allow_output(f"cd {REPO_ROOT} && python3 -m prozesscode.eslint-run | tail -5")
    if both.get("updatedInput", {}).get("command") == "cd . && python3 -m prozesscode.eslint-run":
